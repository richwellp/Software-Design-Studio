...
public class ComplexPlayer implements PlayerStrategy {
    /** Current player's id. */
    ...
    public Card.Suit declareSuit() {
        ...
        int[] occurrences = {diamonds, hearts, spades, clubs};
        // find the highest number of cards that have the same suit
        int maxOccurrence = 0;
        for (int i = 0; i < occurrences.length; i++) {
            if (occurrences[i] > maxOccurrence) {
                maxOccurrence = occurrences[i];
            }
        }
        if (maxOccurrence == diamonds) {
            return Card.Suit.DIAMONDS;
        } else if (maxOccurrence == hearts) {
            return Card.Suit.HEARTS;
        } else if (maxOccurrence == spades) {
